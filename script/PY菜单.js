...
const main_menu = () => {
    if (PY_list.length === 0) {
        clientMessage('§b[DianaGod UI-GB]§e读取脚本列表失败(可能是没有储存权限)');
        return;
    }
    var menu = {
        type: 'form',
...
const settings = () => {
    ...
    addForm(JSON.stringify(json), function(delay, times, mode) {
        ...
        setData('menu_cfg2', JSON.stringify(config))
        main_menu()
    })
}
const load = (script_name) => {
    clientMessage('§b[DianaGod UI-GB]§e加载脚本==> §b' + script_name)
    ...
    setTimeout(() => {
        ...
        clientMessage('§b[DianaGod UI-GB]§ePY加载' + (state ? '成功' : '失败'))
    }, config.delay * 1000);
};
...
var config = JSON.parse((getData('menu_cfg2', '') != '') ? getData('menu_cfg2', '') : JSON.stringify({
    mode: "eval",
    times: 1,
    delay: 0
}))
